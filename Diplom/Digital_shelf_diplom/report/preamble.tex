...
\usepackage{ragged2e} % пакет для нормального выравнивания текста
...
\newcommand{\sectionbreak}{\clearpage} % чтобы каждый раздел начинался с новой страницы
...
\usepackage{lastpage} % для получения номера последней страницы (\pageref{LastPage})

\usepackage{amsmath} % для нормальной работы с формулами

\usepackage{amsfonts} % для отображения русских символов в формулах
...
\setmainlanguage[babelshorthands = true]{russian} % устанавливаем русский как основной язык
...
\setsansfont[Ligatures=TeX]{Arial} % устанавливаем дополнительный шрифт 
...
\newfontfamily\cyrillicfonttt[Script=Cyrillic]{Courier New} % моноширинный шрифт для кириллицы
...
\def\simpleGrad{\noindent\hbox to 0pt{
        \vbox to 0pt{
            \noindent\begin{picture}(185,286.75)(5.5,0.7)
                ...
                \put(64.5, 0){\makebox(109,15){\cyrillicfont \englishfont \fontsize{18pt}{0pt}\selectfont ДП.ИИ24.220086-05 81 00}}
...
\def\@oddfoot{}
\def\@evenhead{\simpleGrad}
\def\@evenfoot{}

% plain используется автоматически на первой странице каждой \chapter
...
% ========== КЛЮЧЕВОЕ ИЗМЕНЕНИЕ ==========
% Устанавливаем счётчик страниц на 0, чтобы первая страница была 1,
% но потом скроем её номер, а на содержании сделаем 3
\setcounter{page}{0}
% =========================================